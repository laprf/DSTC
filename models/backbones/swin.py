...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        layers.append(
            nn.Sequential(
                nn.Conv2d(
                    in_channel, embed_dim, kernel_size=(patch_size[0], patch_size[1]), stride=(patch_size[0], patch_size[1])
                ),
                Permute([0, 2, 3, 1]),
    ...
    **kwargs: Any,
    ...
    if weights is not None:
        ...
        for k, v in state_dict.items():
            if k == 'features.0.0.weight' and kwargs['in_channel'] != 3:
                print(f"in_channels = {kwargs['in_channel']}, jumping features.0.0.weight!")
                continue

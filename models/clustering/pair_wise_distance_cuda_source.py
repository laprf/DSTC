...
__global__ void forward_kernel(
    ...
    const scalar_t* __restrict__ deep_features,
    const scalar_t* __restrict__ sdf_feature,
    ...
    if (x_index + spixel_offset_x < 0 || x_index + spixel_offset_x >= num_spixels_w) {
        ...
        {
            scalar_t pixel_feat = pixel_features[b * cp + c * num_pixels + p];
            scalar_t spixel_feat = spixel_features[b * cs + c * num_spixels + query_spixel_index];
            
            pixel_feat += deep_features[b * cp + c * num_pixels + p];
            pixel_feat += sdf_feature[b * cp + c * num_pixels + p];

            sum_squared_diff += pow(pixel_feat - spixel_feat, 2);
        }
...
torch::Tensor forward_cuda(
    ...
    const torch::Tensor deep_features,
    const torch::Tensor sdf_feature,
    ...
    AT_DISPATCH_FLOATING_TYPES(dist_matrix.type(), "forward_kernel", ([&] {
        forward_kernel<scalar_t><<< block, CUDA_NUM_THREADS >>>(
            ...
            deep_features.data<scalar_t>(),
            sdf_feature.data<scalar_t>(),
...
__global__ void backward_kernel(
    ...
    if (x_index + spixel_offset_x < 0 || x_index + spixel_offset_x >= num_spixels_w) return;
        ...
        for (int c=0; c<channels; c++)
        {
            ...
            pix_value += deep_features[b * cp + c * num_pixels + p];
            pix_value += sdf_feature[b * cp + c * num_pixels + p];
